...
def convert_unstruc_example_to_clingo_format(ex_id, row, preds, net_type):
    gt_label = row[1]
    board = row[0]
    if gt_label == 'valid':
                    ...
                    if d == non_perturbed_dataset and noise_pct > noise_pcts[0]:
                        ...
                        for train_idx in repeats:
                            ...
                            for idx, row in enumerate(csv_file.values):
                                ...
                                ex = convert_unstruc_example_to_clingo_format(idx, row, preds, net_type)
                                examples.append(ex)
                            clingo_examples = '\n'.join(examples)
...
if __name__ == '__main__':
    cmd_args = add_cmd_line_args(desc='Sudoku 4x4 task. Run inference evaluation without problog.',
